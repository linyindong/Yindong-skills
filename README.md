...
The repository currently includes four Codex skills under the Platform Product Skills name:
...
|---|---|---|
| You have not figured out the direction yet | `platform-product-operating-system` | Clarify the business problem, system boundary, reusable platform capability, flow, ownership, and operating model. |
| You are ready to write the document | `platform-product-prd-builder` | Turn rough notes, ideas, meeting notes, or prototypes into a structured PRD/BRD or requirement section. |
| You are unsure whether something belongs in the current phase | `platform-product-scope-governor` | Assess MVP scope, hidden complexity, include/defer decisions, and phase boundaries. |
| The document is written and needs review | `platform-product-prd-reviewer` | Check logic closure, ownership, flow, data/API readiness, edge cases, operations, rollout risk, and open questions. |

In short:

- Not clear on direction yet: use `platform-product-operating-system`
- Ready to write: use `platform-product-prd-builder`
- Unsure whether to include it now: use `platform-product-scope-governor`
- Finished writing and need a review: use `platform-product-prd-reviewer`
...
cp -R skills/platform-product-operating-system ~/.codex/skills/
cp -R skills/platform-product-prd-builder ~/.codex/skills/
cp -R skills/platform-product-prd-reviewer ~/.codex/skills/
cp -R skills/platform-product-scope-governor ~/.codex/skills/
```
...
```text
Use $platform-product-operating-system to reason through this platform requirement.
Use $platform-product-prd-builder to turn these rough notes into a PRD section.
Use $platform-product-prd-reviewer to check whether this PRD is logically complete.
Use $platform-product-scope-governor to assess whether this change belongs in Phase 1.
```

Chinese prompts also work:

```text
用 $platform-product-operating-system 帮我从业务问题到平台能力梳理这个需求。
用 $platform-product-prd-builder 把下面内容整理成英文 PRD。
用 $platform-product-prd-reviewer 检查这份 PRD 是否逻辑闭环。
用 $platform-product-scope-governor 判断这个能力是否应该进 Phase 1。
```
...
```text
Use $platform-product-operating-system to clarify the business problem, system boundary, reusable platform capability, flow, runtime objects, API/data changes, and rollout considerations.
```

For drafting:

```text
Use $platform-product-prd-builder to turn the following notes into a structured PRD. Do not simply translate; reorganize and enrich the content where needed.
```

For review:

```text
Use $platform-product-prd-reviewer to review this PRD. Focus on logic closure, ownership, flow, data model, edge cases, operations, rollout, and open questions.
```

For scope decisions:

```text
Use $platform-product-scope-governor to assess whether this change should be included in the current phase. Give an impact table and a clear include/defer/placeholder recommendation.
```
...
|---|---|---|
| 还没想清楚方向 | `platform-product-operating-system` | 梳理业务问题、系统边界、平台能力、flow、ownership 和运营闭环。 |
| 已经要写文档 | `platform-product-prd-builder` | 把粗略想法、会议记录、原型或零散笔记整理成结构化 PRD/BRD。 |
| 不确定要不要做、或本期做多少 | `platform-product-scope-governor` | 判断 MVP 范围、隐藏复杂度、include/defer、阶段边界。 |
| 文档写完要检查 | `platform-product-prd-reviewer` | 检查逻辑闭环、owner、flow、API/data、edge cases、运营和 rollout 风险。 |

简单记：

- 还没想清楚方向：用 `platform-product-operating-system`
- 已经要写文档：用 `platform-product-prd-builder`
- 不确定要不要做/本期做多少：用 `platform-product-scope-governor`
- 文档写完要检查：用 `platform-product-prd-reviewer`
